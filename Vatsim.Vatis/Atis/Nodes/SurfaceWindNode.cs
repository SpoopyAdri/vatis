...
public class SurfaceWindNode : BaseNode<SurfaceWind>
{
    ...
    public override string ParseVoiceVariables(SurfaceWind node, string format)
    {
        ...
        format = Regex.Replace(format, "{wind_dir}", node.Direction.ApplyMagVar(magVarDeg).NumberToSingular(), RegexOptions.IgnoreCase);
        format = Regex.Replace(format, "{wind_spd}", node.Speed.ToString(leadingZero).NumberToSingular(), RegexOptions.IgnoreCase);
        format = Regex.Replace(format, @"{wind_spd\|kt}", node.ToKts(node.Speed).ToString(leadingZero).NumberToSingular(), RegexOptions.IgnoreCase);
        format = Regex.Replace(format, @"{wind_spd\|mps}", node.ToMps(node.Speed).ToString(leadingZero).NumberToSingular(), RegexOptions.IgnoreCase);
        format = Regex.Replace(format, "{wind_gust}", node.GustSpeed.ToString(leadingZero).NumberToSingular(), RegexOptions.IgnoreCase);
        format = Regex.Replace(format, @"{wind_gust\|kt}", node.ToKts(node.GustSpeed).ToString(leadingZero).NumberToSingular(), RegexOptions.IgnoreCase);
        format = Regex.Replace(format, @"{wind_gust\|mps}", node.ToMps(node.GustSpeed).ToString(leadingZero).NumberToSingular(), RegexOptions.IgnoreCase);
        format = Regex.Replace(format, "{wind_vmin}", node.ExtremeWindDirections?.FirstExtremeDirection.ApplyMagVar(magVarDeg).NumberToSingular() ?? "", RegexOptions.IgnoreCase);
        format = Regex.Replace(format, "{wind_vmax}", node.ExtremeWindDirections?.LastExtremeWindDirection.ApplyMagVar(magVarDeg).NumberToSingular() ?? "", RegexOptions.IgnoreCase);
