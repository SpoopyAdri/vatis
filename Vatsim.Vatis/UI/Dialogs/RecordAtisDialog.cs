...
public partial class RecordAtisDialog : Form
{
    ...
    private readonly System.Timers.Timer mMaxDurationTimer;
    ...
    public MemoryStream AtisMemoryStream { get; }
    ...
    public RecordAtisDialog(IAppConfig appConfig, AtisComposite composite)
    {
        ...
        mMaxDurationTimer.Elapsed += delegate
        {
            ...
            mSyncContext.Post(o =>
            {
                if (MessageBox.Show(this, "Maximum ATIS duration reached (3 minutes). Recording stopped.",
                        "Record ATIS", MessageBoxButtons.OK) == DialogResult.OK)
                {
    ...
    {
        mSyncContext.Post(o => { recordingLength.Text = mRecordingStopwatch.Elapsed.ToString("hh\\:mm\\:ss"); }, null);
    }
    ...
    private void btnCancel_Click(object sender, EventArgs e)
    {
        if (mIsRecording)
        {
            if (MessageBox.Show(this,
                    "You are currently recording an ATIS. Are you sure you want to cancel the operation?",
                    "Cancel ATIS Recording", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                mAtisAudioIn.StopRecording();
    ...
    private void SetInputDevice()
    {
        ...
        {
            AtisMemoryStream.Write(args.Buffer, 0, args.BytesRecorded);
            if (AtisMemoryStream.Position > mAtisAudioIn.WaveFormat.AverageBytesPerSecond * 180)
            {
                mAtisAudioIn.StopRecording();
            }
        };
        ...
        if (ddlInputDeviceName.SelectedValue != null &&
            ddlInputDeviceName.SelectedValue.ToString() != mAppConfig.MicrophoneDevice)
        {
    ...
    private void SetOutputDevice()
    {
        ...
        if (ddlOutputDeviceName.SelectedValue != null &&
            ddlOutputDeviceName.SelectedValue.ToString() != mAppConfig.PlaybackDevice)
        {
    ...
    private void btnListen_Click(object sender, EventArgs e)
    {
        ...
        if (!mPlaybackStarted)
        {
            ...
            IWaveProvider provider = new RawSourceWaveStream(AtisMemoryStream, new WaveFormat(48000, 1));
